...
std::size_t computeColumnResult(const std::vector<std::uint64_t> &numbers,
                                char op) {
...
std::size_t solveLeftToRight(const std::vector<std::string> &rows) {
  std::size_t line_count = rows.size();
...
std::size_t solveRightToLeft(const std::vector<std::string> &rows) {
  ...
  std::vector<char> operators = extractOperators(rows[H - 1]);
  ...
  for (std::size_t i = 0; i < W; i++) {
    ...
    for (std::size_t col = block_left; col <= block_right; col++) {
      std::string num;
      for (std::size_t row = 0; row < H - 1; row++) {
...
int main() {
  ...
  std::cout << "Part 1: " << solveLeftToRight(rows)
            << std::endl; // Correct answer: 4076006202939
  std::cout << "Part 2: " << solveRightToLeft(rows)
